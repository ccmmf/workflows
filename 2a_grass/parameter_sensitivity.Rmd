...
if (!dir.exists("sensitivity")) dir.create("sensitivity")
cat(template_lines[-length(template_lines)], file = "sensitivity/template_sa.xml", sep = "\n")
cat(sa_block, file = "sensitivity/template_sa.xml", append = TRUE, sep = "\n")
cat(template_lines[length(template_lines)], file = "sensitivity/template_sa.xml", append = TRUE, sep = "\n")
```
...
build_site_sa_files <- function(site) {
  site_info <- read.csv("site_info.csv") |>
    filter(id == site)
  write.csv(site_info, paste0("sensitivity/site_info_", site, ".csv"), row.names = FALSE)

  s <- PEcAn.settings::read.settings("sensitivity/template_sa.xml")

  site_out <- paste0("sensitivity/output_", site)
  s$outdir <- site_out
  ...
  callr::rscript(
    ...
    cmdargs = c(
      "--n_ens=1",
      "--n_met=1",
      paste0("--site_file=sensitivity/site_info_", site, ".csv"),
      paste0("--template_file=sensitivity/template_", site, ".xml"),
      paste0("--output_file=sensitivity/settings_", site, ".xml")
    )
  )
}
...
run_site_sa <- function(site) {
  callr::rscript(
    "04_run_model.R",
    cmdargs = paste0("--settings=sensitivity/settings_", site, ".xml")
  )
}
...
var_decomps <- sa_sites |>
  mutate(outdir = paste0("sensitivity/output_", id)) |>
  pull(outdir) |>
