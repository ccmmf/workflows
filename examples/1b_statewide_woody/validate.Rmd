...
agb_fits <- agb_with_lt |>
  ...
  mutate(
    ...
    bias = mean(data$AbvGrndWood - data$AGB_median)
  )
...
allom_fits <- agb_with_allom |>
  ...
  mutate(
    ...
    bias = mean(data$AbvGrndWood - data$allom_C_2023)
  )
...
```{r allom-by-crop}
allom_fits_by_crop <- agb_with_allom |>
  ungroup() |>
  nest_by(species, ens_num) |>
  mutate(
    fit = list(lm(AbvGrndWood ~ allom_C_2023, data = data)),
    r2 = summary(fit)$adj.r.squared,
    nse = 1 - (
...
allom_fits_by_crop |> select(-data, -fit)
allom_fits_by_crop |>
  group_by(species) |>
  summarize(across(r2:bias, c(mean = mean, sd = sd))) |>
  pivot_longer(-species, names_to = c("stat", ".value"), names_sep = "_")
