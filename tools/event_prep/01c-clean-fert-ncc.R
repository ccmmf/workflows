...
# Its path in the PEcAn repository is workflows/preprocess-event-parquet/01c-clean-fertilization.R.
...
options <- list(
  ...
  optparse::make_option("--fert_path",
    ...
    help = "Directory containing Parquet files of fertilization events"
  ),
  optparse::make_option("--ncc_path",
    default = "data_raw/management/ncc/v1.0",
    help = "Directory containing Parquet files of non-crop carbon additions"
  ),
  optparse::make_option("--outdir",
    default = "data/management/fertilization",
    help = paste(
      "Directory to write cleaned fertilization events.",
      "This will include both inorganic (fert) and organic (ncc) nutrient",
      "additions (Sipnet treats both these as fertilization).",
...
has_fert <- length(Sys.glob(file.path(args$fert_path, "*.parquet"))) > 0
has_ncc  <- length(Sys.glob(file.path(args$ncc_path,  "*.parquet"))) > 0
...
DBI::dbExecute(conn, glue::glue("
  ...
  '{args$outdir}/fertilization.parquet'
