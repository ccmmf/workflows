...
options <- list(
  optparse::make_option("--prev_run_dir",
    ...
    # A: So we don't need separate manifest keys for this and copy_prev_restarts.R
  ...
  optparse::make_option("--settings",
    ...
    help = paste(
      "The PEcAn settings file that will be used for the new run.",
      "Sample file will be written to settings$outdir"
    )
  )
) |>
...
if (!dir.exists(new_run_dir)) {
  dir.create(new_run_dir, recursive = TRUE)
}

samp_ok <- file.copy(
  prev_samp_file,
  file.path(new_run_dir, "samples.Rdata"),
  overwrite = TRUE
)
if (!samp_ok) {
  PEcAn.logger::logger.error(
    "Could not copy sample file from previous run",
    file.path(prev_run_dir, "samples.Rdata")
  )
}
